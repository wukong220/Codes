...
class BasePlot:
    ...
    def set_axes(self, ax, data, labels, title, is_3D=False, scatter=False, rotation=-60, loc="right"):
        x, y, z = data[:3]
        ...
        for axis_data, axis_name in zip([x, y, z], ['x', 'y', 'z']):
            if axis_name == 'z' and not is_3D:
                continue
            min_val, max_val = min(axis_data), max(axis_data)
            if scatter:
                getattr(ax, f'set_{axis_name}lim')(min_val * 0.1, max_val * 1.2)
            else:
                getattr(ax, f'set_{axis_name}lim')(min_val, max_val)

    def adding(self, ax, note, is_3D=False):
        # linewidth and note
        ax.annotate(note, (-0.25, 0.9), textcoords="axes fraction", xycoords="axes fraction", va="center", ha="center", fontsize=20)
        if is_3D:
...
class Plotter(BasePlot):
    def scatter(self, fig, ax, data, labels, note, is_3D=False, scatter=True):
        x, y, z, w = data
        ...
        if is_3D:
            sc = ax.scatter(x, y, z, c=w, cmap="rainbow", s=100, vmin=w.min(), vmax=w.max())
            ...
            cbar.ax.set_xlabel(labels[2], fontsize=20, labelpad=10)
        self.set_axes(ax, data, labels, title, is_3D, scatter)
...
class JobProcessor:
    ...
    def Rg2_job(self, Config, Run, iRin):
        ...
        fig_Rg2 = os.path.join(os.path.join(re.match(r"(.*?/Data/)", os.getcwd()).group(1), "Figs"),
        ...
        dir_file = os.path.join(fig_Rg2, f"{abbre}(Pe,N,W)")
        if not os.path.exists(f"{dir_file}.py"):
            shutil.copy2(os.path.abspath(__file__), f"{dir_file}.py")
        print(message)
        logging.info(message)
